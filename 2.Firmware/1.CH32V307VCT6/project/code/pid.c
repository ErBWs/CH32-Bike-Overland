...
#define SERVO_MAX_ANGLE 25.0
	PID_Init(&dirPid,POSITION_PID,SERVO_MAX_ANGLE,0,-(float)0.4,0,0.4);//舵机PD
    ...
    PID_Init(&flyAngleSpdPid,POSITION_PID,PWM_DUTY_MAX-10,PWM_DUTY_MAX-10,-34,-0.8,0);//飞轮角速度环PI
    PID_Init(&backSpdPid,POSITION_PID,PWM_DUTY_MAX-10,3000,80,1,-0.2);//后轮速度环纯P
}
