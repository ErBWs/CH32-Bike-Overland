...
void reset_ubx_decoder(ubx_decoder_t* ubx_decoder)
{
    ...
    /* flush read buffer */
    do {
        ret = uart_query_byte(GPS_TAU1201_UART,  &c);
    } while (ret);
}

err_t init_ubx_decoder(ubx_decoder_t* ubx_decoder, ubx_rx_handle_ptr ubx_rx_handle)
{
...
err_t send_ubx_msg(ubx_decoder_t* ubx_decoder, const uint16_t msg, const uint8_t* payload, const uint16_t length)
{
    ...
    uart_write_buffer(GPS_TAU1201_UART, (const void*)&header, sizeof(header));


    if (payload != NULL) {
        uart_write_buffer(GPS_TAU1201_UART, (const void*)&payload, length);
    }
    
    uart_write_buffer(GPS_TAU1201_UART,(const void*)&checksum, sizeof(checksum));
