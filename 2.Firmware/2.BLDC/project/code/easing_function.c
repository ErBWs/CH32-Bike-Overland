...
float GetEasingCurves(float currentTime, float start, float end, float totalTime, EasingFunction_e func)
{
    ...
    case IN_SIN:
        return -end * (float) cos(currentTime / totalTime * (MATH_PI / 2)) + end + start;
    case OUT_SIN:
        return end * (float) sin(currentTime / totalTime * (MATH_PI / 2)) + start;
    case IN_OUT_SIN:
        return -end / 2 * (float) (cos(MATH_PI * currentTime / totalTime) - 1) + start;
    case IN_EXP:
    ...
    case IN_ELASTIC:
        ...
        if (a < fabsf(end))
        {
            a = end;
            s = p / 4;
        } else s = p / (float) (2 * MATH_PI) * asinf(end / a);
        currentTime -= 1;
        return -(a * (float) pow(2, 10 * currentTime) * (float) sin((currentTime * totalTime - s) * (2 * MATH_PI) / p)) + start;
    case OUT_ELASTIC:
        if (currentTime == 0) return start;
        if ((currentTime /= totalTime) == 1) return start + end;
        ...
        return a * (float) pow(2, -10 * currentTime) * (float) sin((currentTime * totalTime - s) * (2 * MATH_PI) / p) + end + start;
    case IN_OUT_ELASTIC:
        if (currentTime == 0) return start;
        ...
        if (currentTime < 1)
        {
            return -0.5f * (a * (float) pow(2, 10 * currentTime) * (float) sin((currentTime * totalTime - s) * (2 * MATH_PI) / p)) + start;
        }
        return a * (float) pow(2, -10 * currentTime) * (float) sinf((currentTime * totalTime - s) * (float) (2 * MATH_PI) / p) * 0.5f + end + start;
    case IN_BACK:
        currentTime /= totalTime;
